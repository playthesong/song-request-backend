...
import live.playthesong.songrequest.global.error.exception.business.LetterNotFoundException;
...
class LetterServiceTest {
    ...
    void find_all_letters() {
        // given
        PaginationParameters parameters = PaginationFactory.createPaginationParameters();
        PageRequest pageRequest = PaginationFactory.createPageRequest();
    ...
    void find_all_letters_by_request_status() {
        ...
        int first = 0;
        int dayAgo = 1;
        PaginationParameters paginationParameters = PaginationFactory.createPaginationParameters();
        ...
        when(letterRepository.findAllTodayLettersByRequestStatus(refEq(pageRequest), eq(RequestStatus.WAITING),
                                                                 any(LocalDateTime.class), any(LocalDateTime.class)))
                .thenReturn(new PageImpl<>(waitingLetters));
        LettersResponse waitingLettersResponse = letterService.findLettersByStatus(RequestStatus.WAITING, paginationParameters, dateParams);
    ...
    void create_new_letter(String songStory, SongRequest songRequest, Long accountId) {
        ...
        OAuthAccount loginAccount = createOAuthAccountOf(accountId, Role.MEMBER);
        Account account = createMember();
        Song song = createSong();
    ...
    void account_mismatch_update_letter(Long letterAccountId, Long differentId, String songStory) {
        ...
        OAuthAccount loginAccount = createOAuthAccountOf(differentId, Role.MEMBER);
    ...
    void change_status() {
        // given
        Letter waitingLetter = createLetterOf(RequestStatus.WAITING, createMember(), createSong());
        StatusChangeRequest doneRequest = createStatusChangeRequestOf(RequestStatus.DONE);
